...
const AddToPaymentModal: React.FC<Props> = ({ visible, onClose, product, onAddToCart }) => {
  ...
  const handleAddToCart = async () => {
    ...
    // console.log(selectedAttributes, product.id, quantity);
    ...
    try {
        ...
        //   });
          ...
          navigation.navigate('payment', { product: [{product_id: selectedVariant?._id || "1", quantity, type: selectedAttributes.map(attr => `${attr.category}: ${attr.value}`).join(', ')}], paymentMethodId: '681a1bde3427154ae2166ebd' });
      onClose();
      setSelectedOptions({});
      setQuantity(1);
      // setoutOfStock(false)
...
export default AddToPaymentModal;
