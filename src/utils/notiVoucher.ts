...
export const checkAndNotifyExpiringVouchers = async () => {
  try {
    ...
    if (res?.data?.EC === 0) {
      ...
      const thumbnail = expiringVouchers[0]?.image || ''; // dùng ảnh của voucher đầu tiên
    // //   senderId, receiveId, name, image, description, fcmToken,
    //   for (const voucher of expiringVouchers) {
    //     await createNotiVoucher( user._id, randomName, voucher.image, randomDec, user.fcm_token);
    //   }

      // if (expiringVouchers.length > 0) {
      //   console.log("Đã gửi thông báo voucher sắp hết hạn.");
      // }
      await createNotiVoucher( user._id, randomName, thumbnail,
         randomDec, user.fcm_token);
      console.log('Tạo thông báo voucher sắp hết hạn thành công.');
      }
    }
  } catch (err) {
    console.log('Lỗi khi tạo thông báo voucher:', err);
  }
};
