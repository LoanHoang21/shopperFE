...
const SearchScreen = () => {
    ...
    useEffect(() => {
        fetch('http://192.168.79.11:3001/api/product/suggested/ml-trending')
            .then(res => res.json())
            .then(json => {
                if (json.status === 'OK') {
    ...
    const handleSearch = async (keyword: string) => {
        try {
            const res = await fetch(`http://192.168.79.11:3001/api/product/search?query=${encodeURIComponent(keyword)}`);
